...
const CoreConfigHelper = stampit({
  init (root) {
    const config = require(`${root}/config/config.json`);
    const confMessagesUri = `${__dirname}/i18n/${config.defaultLanguage}.confManMessages.json`;
    ...
    require(`${__dirname}/i18n/${config.defaultLanguage}.confManMessages.json`),
    { logger }, config);
  }
});

module.exports = root => CoreConfigHelper(root);
